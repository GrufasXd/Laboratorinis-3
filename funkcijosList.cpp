...
void clearFiles() {
    ofstream ofs1("vargsai.txt", ofstream::out | ofstream::trunc);
    ofstream ofs2("galva.txt", ofstream::out | ofstream::trunc);
    if (!ofs1 || !ofs2) {
        cerr << "Error: Unable to clear files." << endl;
    } else {
        ofs1.close();
        ofs2.close();
    }
...
double calculateMedian(const studentas& s) {
    list<int> sorted_nd(s.nd.begin(), s.nd.end());
    sorted_nd.sort();
    int n = sorted_nd.size();
    auto it = sorted_nd.begin();
    advance(it, n / 2);
    
    if (n % 2 == 0) {
        int first_middle = *prev(it);
        int second_middle = *it;
